...
my $config = do($ENV{'HOME'} . '/.enum4linuxrc');
if ($config) {
    if ($config->{smbclient_cmd}) {
        $smbclient_cmd = $config->{smbclient_cmd};
        print "Using smblclient at $smbclient_cmd\n";
    }
}
...
sub make_session {
    ...
    my $command = "$smbclient_cmd -W '$global_workgroup' //'$global_target'/ipc\$ -U'$global_username'\%'$global_password' -c 'help' 2>&1";
...
sub get_os_info {
    ...
    my $command = "$smbclient_cmd -W '$global_workgroup' //'$global_target'/ipc\$ -U'$global_username'\%'$global_password' -c 'q' 2>&1";
...
sub enum_shares {
    ...
    my $command = "$smbclient_cmd -W '$global_workgroup' -L //'$global_target' -U'$global_username'\%'$global_password' 2>&1";
    my $shares = `$command`;
    if (defined($shares)) {
    ...
    foreach my $share (@shares) {
        $share =~ s/'/'\\''/g;
        my $command = "$smbclient_cmd -W '$global_workgroup' //'$global_target'/'$share' -U'$global_username'\%'$global_password' -c dir 2>&1";
...
sub enum_shares_unauth {
    ...
    foreach my $share (@shares) {
        ...
        my $result = `$smbclient_cmd -W '$global_workgroup' //'$global_target'/'$share' -c dir -U'$global_username'\%'$global_password' 2>&1`;
